...
class MaximumProfitOfOperatingACentennialWheel {
    ...
    class Solution {
        /**
         * 每一轮运行完成之后计算截止到当前时的盈利，然后取盈利最大值所在的轮数即可
         *
         * <p>
         * 前缀和
         *
        ...
        public int minOperationsMaxProfit(int[] customers, int boardingCost, int runningCost) {
            ...
            for (int i = 0; i < customers.length; i++) {
                // 等待游客大于等于4时，直接运行一轮
                if (customers[i] >= 4) {
                    int times = customers[i] / 4;
                    lastProfit = (boardingCost * 4 - runningCost) * times + lastProfit;
                    runTimes += times;
                    if (lastProfit > max) {
                        max = lastProfit;
                        result = runTimes;
                ...
                // 等待游客小于4时
                // 如果此时运行轮数不大于i，或者已经没有下一轮游客，则必须运行一轮
                if (runTimes <= i || i == customers.length - 1) {
                    lastProfit = boardingCost * customers[i] - runningCost + lastProfit;
                    runTimes++;
                    if (lastProfit > max) {
                        result = runTimes;
                    }
